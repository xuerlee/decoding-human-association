...
def collective_path(fm_root, ann_root):
    ...
    # val_seqs = [str(i + 2) for i in range(32)]
...
class FeatureMapDataset(data.Dataset):
    ...
    #         device = torch.device('cpu')
        ...
        for i, (sid, src_fid, fid) in enumerate(select_frames[2]):  # 10 frames for 1 item
            ...
            # TODO: maybe no need to do the reduce of channels
            featuremaps.append(feature_map)

        # labels for the whole video clip (the label of the key frame)
        meta = {}
        featuremaps = np.stack(featuremaps)
        bboxes = np.array(bboxes, dtype=np.float64).reshape(-1, 4)
        actions = np.array(actions, dtype=np.int32)
        activities = np.array(activities, dtype=np.int32)
        one_hot_matrix = np.array(one_hot_matrix, dtype=np.int32)

        featuremaps = torch.from_numpy(featuremaps).float()
        featuremaps = torch.squeeze(featuremaps, 1)  # shape: (10, 1392, 31, 46) (10, 1392, 31, 51)
        bboxes = torch.from_numpy(bboxes).float()
        actions = torch.from_numpy(actions).long()
        activities = torch.from_numpy(activities).long()
        one_hot_matrix = torch.from_numpy(one_hot_matrix).int()

        FH = featuremaps.shape[2]
        FW = featuremaps.shape[3]
        OH = FRAMES_SIZE[sid][0]
        OW = FRAMES_SIZE[sid][1]
        H_ratio = FH / OH
        W_ratio = FW / OW
        meta['down_sample_ratio'] = [H_ratio, W_ratio]
        meta['sid'] = sid
        meta['src_fid'] = src_fid
        meta['frame_size'] = FRAMES_SIZE[sid]
        meta['featuremap_size'] = [FH, FW]

        return featuremaps, bboxes, actions, activities, one_hot_matrix, meta
