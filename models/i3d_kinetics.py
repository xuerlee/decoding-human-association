...
class Unit3D(nn.Module):
    ...
    def __init__(self, in_channels,
        ...
        if self._use_batch_norm:
            self.batch3d = nn.BatchNorm3d(self._output_channels, eps=0.001, momentum=0.01)
...
class InceptionI3d(nn.Module):  #  output mixed_4b / mixed_3c features + mixed_5c/... global features
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
                 final_endpoint='conv3d_0c_1x1', name='inception_i3d', in_channels=3, dropout_keep_prob=0.5):
        """Initializes I3D model instance.
        Args:
          num_classes: The number of outputs in the logit layer (default 400, which
              matches the Kinetics dataset).
          spatial_squeeze: Whether to squeeze the spatial dimensions for the logits (conv3d_0c_1x1)
          ...
          final_endpoint: The model contains many possible endpoints.
              ...
              InceptionI3d.VALID_ENDPOINTS (default 'Logits (conv3d_0c_1x1)').
        ...
        end_point = 'conv3d_0c_1x1'
        self.avg_pool = nn.AvgPool3d(kernel_size=[2, 7, 7],
                                     stride=(1, 1, 1))
        self.dropout = nn.Dropout(dropout_keep_prob)
        self.conv3d_0c_1x1 = Unit3D(in_channels=384 + 384 + 128 + 128, output_channels=self._num_classes,
                             kernel_shape=[1, 1, 1],
                             padding=0,
                             activation_fn=None,
    ...
    def set_param(self):
        # for end_point in self.INVALID:
        #     for param in self.end_points[end_point].parameters():
        #         param.requires_grad=False
        for param in self.conv3d_0c_1x1.parameters():
            param.requires_grad = False
    ...
    def forward(self, x):
        ...
        # x = self.conv3d_0c_1x1(self.dropout(self.avg_pool(x)))
...
class InceptionI3d_noglobal(nn.Module):  #  output mixed_4b / mixed_3c features without 5b/5c updating
    """Inception-v1 I3D architecture.
    The model is introduced in:
        Quo Vadis, Action Recognition? A New Model and the Kinetics Dataset
        Joao Carreira, Andrew Zisserman
    ...
    def __init__(self, num_classes=400, spatial_squeeze=True,
        ...
        super(InceptionI3d_noglobal, self).__init__()
    ...
    def forward(self, x):
        ...
        for end_point in self.VALID_ENDPOINTS:  # including prediction and logits (conv3d_0c_1x1)
            if end_point == 'MaxPool3d_5a_2x2':  # can output features from other layers
                break
            # if end_point == 'MaxPool3d_4a_3x3':
            #    break
            if end_point in self.end_points:  # without prediction and logits (conv3d_0c_1x1)
                x = self._modules[end_point](x)  # use _modules to work with dataparallel (output mixed_4f layer)
...
class i3d_noglobal(nn.Module):
    def __init__(self, in_channel=832, out_channel=256):
        ...
        self.i3d.load_state_dict(torch.load(
            # 'models/pretrained_models/rgb_charades.pt'))
            'models/pretrained_models/model_rgb.pth'))
        # self.i3d.replace_logits(8)
