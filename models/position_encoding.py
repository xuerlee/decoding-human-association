...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, bbox, n_max, n_per_frame):  # bbox: N, 4
        B = len(n_per_frame)  # in evaluation last batch (no enough samples for one batch)
        pos = self.pos_fc(bbox).reshape(-1, self.hidden_dim)  # N, hidden_dim(256)
        pos_pad = torch.zeros((B * n_max, self.hidden_dim), device=pos.device)
        start = 0
        for i, n in enumerate(n_per_frame):
            pos_pad[i*n_max: i*n_max+n, :].copy_(pos[start: start+n, :])
            start += n
        pos_pad = pos_pad.reshape(B, n_max, self.hidden_dim).permute(1, 0, 2).contiguous()  # n_max, B, hidden_dim
        return pos_pad
