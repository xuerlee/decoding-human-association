...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, bbox, n_max, n_per_frame):  # bbox: N(with T), 4
        ...
        pos_pad = torch.zeros((B * n_max, self.T, self.hidden_dim), device=pos.device)
        start = 0
        for i, n in enumerate(n_per_frame):
            pos_pad[i*n_max: i*n_max+n, :, :].copy_(pos[start: start+n, :, :])
            start += n
        pos_pad = pos_pad.reshape(B, n_max, self.T, self.hidden_dim).permute(0, 2, 1, 3).reshape(B*self.T, n_max, self.hidden_dim).permute(1, 0, 2)
        return pos_pad
