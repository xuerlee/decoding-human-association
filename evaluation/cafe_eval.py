...
def outlier_metric_from_onehot(all_oh, all_aw):
    """
    ...
    GT: singleton/outlier = rows with all-zero across groups
    Pred: singleton/outlier =
          predicted group size == 1 (fallback, not as good)
    Return: mean IoU over clips * 100
    """
    ious = []
    for oh, aw in zip(all_oh, all_aw):
        P = oh.shape[0]
        if P == 0:
            continue
        ...
        # ---- Pred outliers: predicted group size == 1 ----
        ...
        iou = 1.0 if union == 0 else inter / (union + 1e-8)
        ious.append(iou)

    return float(np.mean(ious) * 100.0) if len(ious) else float("nan")
...
def group_mAP_eval(gt_groups_ids, gt_groups_activity, pred_groups_ids, pred_groups_activity, pred_groups_scores,
    ...
    for c, clas in enumerate(categories):
        ...
        pred_groups = sorted(pred_groups, key=lambda conf: next(iter(conf[3])), reverse=True)
